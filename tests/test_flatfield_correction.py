...
class TestBkgSubtraction(unittest.TestCase):
    def test_subtract(self):
        im = da.from_array(
            np.full((64, 64, 64), fill_value=100, dtype=np.uint16),
            chunks=(16, 16, 16),
        )
        bkg_im = da.from_array(
            np.full((32, 32), fill_value=50, dtype=np.uint16), chunks=(16, 16)
        )
        ...
        bkg_im = da.from_array(
            np.full((128, 128), fill_value=50, dtype=np.uint16),
            chunks=(16, 16),
        )
        result = BkgSubtraction.subtract(im, bkg_im)
        self.assertIsInstance(result, da.Array)
        ...
        bkg_im = da.from_array(
            np.full((64, 64), fill_value=200, dtype=np.uint16), chunks=(16, 16)
        )
        result = BkgSubtraction.subtract(im, bkg_im)
        self.assertIsInstance(result, da.Array)
        self.assertEqual(result.shape, im.shape)
