...
if __name__ == "__main__":
    ...
    job_configs = (
        EphysJobConfigurationLoader().load_configs(args.conf_file_location)
    )
    ...
    if job_configs["clip_data_job"]["clip"]:
        ...
        streams_to_clip = EphysReaders.get_streams_to_clip(data_name,
                                                           data_src_dir)
        EphysWriters.copy_and_clip_data(data_src_dir,
                                        clipped_data_path,
                                        streams_to_clip,
                                        n_frames)
    ...
    if job_configs["compress_data_job"]["compress"]:
        ...
        compressor_name = (
            job_configs["compress_data_job"]["compressor"]["compressor_name"])
        compressor_kwargs = (
            job_configs["compress_data_job"]["compressor"]["kwargs"])
        ...
        compressor = EphysCompressors.get_compressor(compressor_name,
                                                     compressor_kwargs)
    ...
    if job_configs["upload_data_job"]["upload_to_s3"]:
        aws_compressed_dest = (
                job_configs["upload_data_job"]["s3_dest"] +
                "/ecephys_compressed")
        aws_clipped_dest = (
                job_configs["upload_data_job"]["s3_dest"] +
                "/ecephys_clipped")
        ...
        if job_configs["upload_data_job"]["dryrun"]:
            subprocess.run(['aws', 's3', 'sync', clipped_data_path,
                            aws_clipped_dest, '--dryrun'])
            subprocess.run(['aws', 's3', 'sync', compressed_data_path,
                            aws_compressed_dest, '--dryrun'])
        else:
            subprocess.run(['aws', 's3', 'sync', clipped_data_path,
                            aws_clipped_dest])
            subprocess.run(['aws', 's3', 'sync', compressed_data_path,
    ...
    if job_configs["upload_data_job"]["upload_to_gcp"]:
        gcp_compressed_dest = (
                job_configs["upload_data_job"]["gcp_dest"] +
                "/ecephys_compressed")
        gcp_clipped_dest = (
                job_configs["upload_data_job"]["gcp_dest"] +
                "/ecephys_clipped")
        clipped_data_path = job_configs["clip_data_job"][
            "clipped_data_dest"]
        compressed_data_path = (
            job_configs["compress_data_job"]["compressed_data_dest"])
        if job_configs["upload_data_job"]["dryrun"]:
            subprocess.run(['gsutil', '-m', 'rsync', '-n',
                            clipped_data_path, gcp_clipped_dest])
            subprocess.run(['gsutil', '-m', 'rsync', '-n',
                            compressed_data_path, gcp_compressed_dest])
        else:
            subprocess.run(['gsutil', '-m', 'rsync', clipped_data_path,
                            gcp_clipped_dest])
            subprocess.run(['gsutil', '-m', 'rsync', compressed_data_path,
                            gcp_compressed_dest])
