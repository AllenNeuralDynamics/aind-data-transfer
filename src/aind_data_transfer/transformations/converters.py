...
#Zeiss LS 7 metadata reader util functions
#########################################
...
def get_instrument_id_from_czi_mdata(mdata):
    instrument_id = "Zeiss " + mdata.czi_box.ImageDocument.Information.Instrument.Microscopes.Microscope.System + ", " + mdata.czi_box.ImageDocument.Metadata.Information.Instrument.Microscopes.Microscope.Id
...
def get_detector_name(mdata):
    detector_model = mdata.czi_box.ImageDocument.Metadata.Information.Instrument.Detectors.Detector[0].Manufacturer.Model
    detector_id = mdata.czi_box.ImageDocument.Metadata.Information.Instrument.Detectors.Detector[0].Id
    detector_type = mdata.czi_box.ImageDocument.Metadata.Information.Instrument.Detectors.Detector[0].Type
...
def get_tiff_tile_name(mdata, tile_index, list_of_tiles):
    ...
    #channel wavelength 
    channel_wavelength = int(float(mdata.image.czisource.ImageDocument.Metadata.Information.Image.Dimensions.Channels.Channel[channel_number].IlluminationWavelength.SinglePeak))
...
def get_schema_AcquisitionTile(mdata, tile_index, list_of_tiles):
    ...
    excitation_wavelength = float(mdata.czi_box.ImageDocument.Metadata.Information.Instrument.LightSources.LightSource[0].LightSourceType.Laser.Wavelength)
...
##################
# Zeiss LS 7 Metadata reader
##################

def get_tile_resolution_from_xml(xml_loc):
    with open(xml_loc, 'r') as f:
        data: xmltodict.parse(f.read())
    resolution_str = data["SpimData"]["SequenceDescription"]["ViewSetups"]["ViewSetup"][0]["voxelSize"]["size"]
    resolution_xyz = [float(x) for x in resolution_str.split(" ")]
    return tuple(resolution_xyz[::-1])

def make_czi_acquisition_schema(czi_loc):
    ...
    list_of_tiles = sorted(czi_loc.glob('*.czi'))

    #first mdata
    mdata = czimd.CziMetadata(list_of_tiles[0])



    specimen_id = get_subject_id_from_dataset_loc(dataset_loc)
    subject_id = get_subject_id_from_dataset_loc(dataset_loc)#mouse id - can get this from title
