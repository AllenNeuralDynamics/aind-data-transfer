...
def parse_json(json_path: str, s3_data_path:str) -> ET.ElementTree:
    ...
    def extract_tile_names_unaltered(
        json_dict: dict
    ) -> list[str]:
    ...
    def add_sequence_description(
        ...
        def add_image_loader(
            seq_desc: ET.Element, tiles: list[str], 
            s3_data_path: str,
        ) -> None:
    ...
    def add_view_registrations(
        ...
        for i, tr in enumerate(translations):
            ...
            affine.text = (f"1.0 0.0 0.0 {str(float(tr[0]))} " +
    ...
    tile_resolution = extract_tile_resolution(json_dict)
    ...
    add_sequence_description(spim_data, tile_names, tile_sizes, tile_resolution, tile_channel_numbers, s3_data_path)
...
if __name__ == '__main__':
    # tree = parse_json('old_ref_r12/old_ref_r12.edited.json')
    # write_xml(tree, 'r12_updated.xml')
    
    # argparser = argparse.ArgumentParser()
    # argparser.add_argument('dataset_loc', type=str)
    ...
    write_xml(tree, test_loc+ '/stitching_405_channels.xml')
    write_xml(tree_rc, test_loc+ '/stitching_405_radial_correction.xml')
